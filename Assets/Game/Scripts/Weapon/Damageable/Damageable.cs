...
public abstract class Damageable : MonoBehaviourPun
{
    ...
    public HitData OnDamageTakenInvoker(int dmg, Collider collider)
    {
        photonView.RPC(nameof(OnDamageTakenShare), RpcTarget.All, dmg, Array.IndexOf(_colliders, collider));
        return OnDamageTaken(dmg, Array.IndexOf(_colliders, collider));
    }
    ...
    protected abstract HitData OnDamageTaken(int dmg, int colliderIndex);
}